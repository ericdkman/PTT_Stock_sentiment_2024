...
#             kick_other_session=True:是用來在當此帳號在其他裝置上已經先登入時，將其登出從此裝置登入
    ...
    def get_num_newest_posts(self, post_num):
        ...
        newest_index = self.ptt_bot.get_newest_index(
            index_type=data_type.NewIndex.BOARD, board='Stock', search_type=data_type.SearchType.KEYWORD, search_condition="盤中閒聊")
...
#       將例外進行排除
        ...
        pushes_df.rename(
            columns={'type': 'Tag', 'author': 'Userid', 'content': 'Content', 'ip': 'Ip', 'time': 'Ipdatetime'}, inplace=True)
...
if __name__ == '__main__':
    ...
    pyptt_obj.generate_newest_posts_dataset(
        post_num=10, save_path='ptt_stock_posts_num1.csv')
    # 登出
    pyptt_obj.logout()
